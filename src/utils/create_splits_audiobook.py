...
def create_splits(subset_dir: str, train_ratio: float):
    ...
    methods = {}
    for item in data:
        for method in item['audio']['fake'].keys():
    ...
    methods_to_test = ["Chatterbox", "FishTTS", "Llasa-1B", "Llasa-1B-Multilingual", "Llasa-3B", "Llasa-8B", "MatchaTTS", "Metavoice-1B", "OpenVoiceV2", "f5-tts", "kokoro", "parler_tts_large_v1", "parler_tts_mini_v0.1", "parler_tts_mini_v1"]
    test_split = []
    rest_split = []

    for item in data:
        test_fake_audio = {method: audio for method, audio in item['audio']['fake'].items() if method in methods_to_test}
        rest_fake_audio = {method: audio for method, audio in item['audio']['fake'].items() if method not in methods_to_test}
    ...
    print(f"Number of test methods: {len(methods_to_test)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--subset_dir", type=str, default="data/QuadVoxBench/Audiobook", help="Path to the subset directory")
