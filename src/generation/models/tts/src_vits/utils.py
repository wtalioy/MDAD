...
def plot_spectrogram_to_numpy(spectrogram):
    ...
    if not MATPLOTLIB_FLAG:
        ...
        # Disable matplotlib logging with loguru
...
def get_logger(model_dir, filename="train.log"):
    # With loguru, we can add a file handler directly
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    
    # Add file handler to loguru
    log_path = os.path.join(model_dir, filename)
    logger.add(log_path, level="DEBUG", format="{time:YYYY-MM-DD HH:mm:ss} | {level: <8} | {name}:{function}:{line} - {message}")
    return logger
